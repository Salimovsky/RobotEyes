...
class FaceTrackerFactory implements MultiProcessor.Factory<Face> {
    ...
    private final RangeFinderAgentFactory rangeFinderAgentFactory;

    FaceTrackerFactory(GraphicOverlay graphicOverlay, RangeFinderAgentFactory rangeFinderAgentFactory) {
        mGraphicOverlay = graphicOverlay;
        this.rangeFinderAgentFactory = rangeFinderAgentFactory;
    }

    @Override
    public Tracker<Face> create(Face face) {
        final FaceGraphic graphic = new FaceGraphic(mGraphicOverlay);
        final RangeFinderAgent rangeFinderAgent = rangeFinderAgentFactory.createNewAgent(graphic);
        return new GraphicTracker(mGraphicOverlay, graphic, rangeFinderAgent);
    }
}
...
class FaceGraphic extends TrackedGraphic<Face> {
    ...
    private static final float DISTANCE_Y_OFFSET = 100.0f;
    private static final float DISTANCE_X_OFFSET = -100.0f;
    ...
    private volatile int distanceInCentimeter;
    ...
    void updateItemDistance(int distanceInCentimeter) {
        this.distanceInCentimeter = distanceInCentimeter;
        postInvalidate();
    }
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.drawText("distance: " + distanceInCentimeter + "cm", cx + DISTANCE_X_OFFSET, cy + DISTANCE_Y_OFFSET, mIdPaint);
